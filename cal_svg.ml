...
let () =
  ...
  add_text svg ~x:10 ~y:22 ~text_anchor:"left" ~font_family:"sans-serif" ~font_size:9.0 ~font_weight:"bold" ~fill:"#444" ~text;
  add_newline svg;
  (* Labels: days names *)
  for i = 0 to 6 do
    let x = 10 + i * 40 in
  ...
  for w = 0 to num_rows do  (* for each row of the month *)
    for i = 0 to 6 do  (* for each day *)
      ...
      if d = 0 then begin
        ...
        add_text svg ~x:(x+2) ~y:(y+6) ~text_anchor:"right" ~font_family:"sans-serif" ~font_size:5.0 ~font_weight:"normal" ~fill:"#222" ~text;
      end;
    done;
    add_newline svg;
  done;
