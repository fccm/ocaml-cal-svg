...
let () =
  ...
  for i = 0 to 11 do
    ...
    let width = 210 in
    let x = float i *. (float width /. 12.0) in
    let w = (float width /. 12.0) in
    add_rect svg ~x ~y:0.0 ~width:w ~height:32.0 ~fill ~fill_opacity:0.5 ();
  done;
  add_newline svg;

  (* Year and title *)
  let text = Printf.sprintf "%s %d" (String.capitalize_ascii cal) year in
  add_text svg ~x:12 ~y:70 ~text_anchor:"left" ~font_family:"sans-serif" ~font_size:9.4 ~font_weight:"bold" ~fill:"#666" ~text ();
  add_newline svg;
  ...
  for mon = 1 to 12 do
    ...
    begin_group svg ~translate:(tx, ty) ~scale:(0.25, 0.25);
    add_newline svg;
    ...
    add_rect svg ~x:0.0 ~y:6.0 ~width:225.0 ~height ~rx:14.0 ~ry:14.0 ~fill_opacity:0.6 ~fill ();
